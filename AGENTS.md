...
5. **Dual-linking.** Every cross-reference uses both formats on the same line: `[[slug|Name]] ([Name](../category/slug.md))`. Obsidian reads the wikilink, the agent reads the markdown link, GitHub renders it. Not locked into any tool. A bare `[[wikilink]]` with no Markdown sibling is dead plain text outside Obsidian — lint flags it (C4c).
